defmodule BlogWeb.PasswordResetControllerTest do
  use BlogWeb.ConnCase

  alias Blog.RegistrationReset
  ...
  def fixture(:password_reset) do
    {:ok, password_reset} = RegistrationReset.create_password_reset(@create_attrs)
    password_reset
  end

  describe "index" do
    test "lists all new", %{conn: conn} do
      conn = get(conn, Routes.password_reset_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing New"
    end
  end
  ...
  describe "create password_reset" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.password_reset_path(conn, :show, id)
  ...
  describe "edit password_reset" do
    ...
    test "renders form for editing chosen password_reset", %{
      conn: conn,
      password_reset: password_reset
    } do
      conn = get(conn, Routes.password_reset_path(conn, :edit, password_reset))
  ...
  describe "update password_reset" do
    ...
    test "redirects when data is valid", %{conn: conn, password_reset: password_reset} do
      ...
      assert redirected_to(conn) == Routes.password_reset_path(conn, :show, password_reset)

      conn = get(conn, Routes.password_reset_path(conn, :show, password_reset))
  ...
  describe "delete password_reset" do
    ...
    test "deletes chosen password_reset", %{conn: conn, password_reset: password_reset} do
      ...
      assert redirected_to(conn) == Routes.password_reset_path(conn, :index)
